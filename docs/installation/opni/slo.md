...
which lets consumers of Opni SLOs specify the good events and total events they consider relevant
to their applications.

For example, should an operator want to observe the reliability of an application's **http requests**,
...
4. Select a metric exposed by your system that you want to monitor with an SLO.
...
8. The target for the [SLO](#service-level-objective-slo) to achieve over the period.
...
**Opni alerting provides some comprehensive dashboards to monitor the status of your SLOs via [Grafana that is set up by Opni Monitoring](./installation/opni/backends#grafana)**

### Multi-Tenancy

Opni Alerting SLOs support multi-tenancy via opni-monitoring grafana access control, see [here](./backends.md#access-control).
...
After setting up your access control, you can head to your Opni cluster's Grafana deployment and open the `Slo - Overview` and `Slo - Detailed` for thorough dashboard breakdowns on your SLO's performance.
...
Ability to filter by cluster, service and name when viewing metrics of your SLOs.
...
A remaining error budget of 100% means that the service has not encountered any bad events or failures.
...
**The estimated expected consumption of the budget over the SLI's period based on the observed error budget and burn rate.**
